...
export const config = {
  ...
  appOrigin: process.env.APP_ORIGIN ?? "http://127.0.0.1:43147",
  sessionSecret: process.env.SESSION_SECRET ?? (isProd ? "" : defaultDevSecret),
  sessionDays: Number(process.env.SESSION_DAYS ?? 7),
  cookieName: "gf_session",
  adminEmail: (process.env.ADMIN_EMAIL ?? "tania@ena.pt").trim().toLowerCase(),
  adminPassword: process.env.ADMIN_PASSWORD ?? (isProd ? "" : defaultDevPassword),
  ...
  pgliteDir: process.env.PGLITE_DIR ?? new URL("../data/pglite", import.meta.url).pathname,
