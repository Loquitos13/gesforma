...
export function linesToXml(linhas: string[], cta: string): string {
  const ps = linhas
    ...
    .join("\n");
  return `<email>\n${ps}\n  <cta>${escapeXml(cta)}</cta>\n</email>\n`;
}

export function parseEmailXml(xml: string): EmailDoc {
  const src = xml?.trim() || "";
  const cta = unescapeXml((src.match(/<cta\b[^>]*>([\s\S]*?)<\/cta>/i)?.[1] ?? "").trim());
  const paragraphs = [...src.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi)].map(m => innerToPlain(m[1]));
  if (paragraphs.length) return { paragraphs, cta };
  if (!src) return { paragraphs: [], cta };
  return { paragraphs: [src.replace(/<\/?email>|<\/?cta\b[^>]*>[\s\S]*$/gi, "").trim()].filter(Boolean), cta };
}
...
export function xmlToLines(xml: string): { linhas: string[]; cta: string } {
  const doc = parseEmailXml(xml);
  return { linhas: doc.paragraphs, cta: doc.cta };
}
